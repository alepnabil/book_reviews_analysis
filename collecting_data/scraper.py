...
class Goodreadscraper():
    ...
    def __init__(self, url: str, book_name: str, language: str):
        ...
        self.chromeoption.add_argument('--ignore-ssl-errors=yes')
        self.chromeoption.add_argument('--ignore-certificate-errors')

        #to evade detection as bot
        # self.chromeoption.add_argument('--disable-blink-features=AutomationControlled')

        #to avoid from chrome crashing
        self.chromeoption.add_argument('--disable-dev-shm-usage')
        ...
        self.driver = webdriver.Remote(command_executor='http://127.0.0.1:4445/wd/hub',
                                       options=self.chromeoption,
    ...
    def save_data(self, data: pd.DataFrame) -> None:
        ...
        if self.page_count == 1:
            ...
            self.logger.info('---DONE SAVING TO CSV FOR FIRST PAGE--')
        elif self.page_count == 2:
            data.to_csv(f'raw_data/{self.language}/{self.book_name}.csv', mode='a', index=False, header=False ,
                        encoding='utf-8')
            self.logger.info('---DONE SAVING TO CSV FOR REMAINING PAGES--')
